                        ...
                        <?php else: ?>
                            <?php foreach ($prioridades as $p): ?>
                                <tr>
                                    ...
                                    <td class="text-center" style="background-color: <?= htmlspecialchars($p->getCor()) ?>; color: #222; font-weight: bold;">
                                        <span class="badge rounded-pill px-3 py-2 border shadow-sm"
                                              style="background: #fff; color: #222; font-size: 1rem;">
                                            <?= htmlspecialchars($p->getCor()) ?>
                                        </span>
                                    </td>
                                    <td class="text-center">
                                        <div class="d-flex flex-wrap justify-content-center gap-1">
                                            <a href="alterar.php?id=<?= $p->getId() ?>" class="btn btn-sm btn-primary flex-fill">
                                                <i class="bi bi-pencil mx-1"></i>Editar
                                            </a>
                                            <a href="excluir.php?id=<?= $p->getId() ?>" class="btn btn-sm btn-danger flex-fill"
                                               onclick="return confirm('Tem certeza que deseja excluir <?= $p->getNivel() ?>?');">
